		...
		<table class="table">
			...
			<tbody>
				...
				<?php if($requerimiento->req_default == 1){ 
					$valor = $cantidad_produccion*$requerimiento->req_ratio;

				?>	

				<td>
					<input type="text" name="req_total[]" id="val-{{$requerimiento->req_ingrediente}}" class="form-control" value="{{$valor}}"  autocomplete="off" >
				</td>			
				<?php }else{ ?>		
				<td>
					<input type="text" name="req_total[]" id="val-{{$requerimiento->req_ingrediente}}" class="form-control" value=""  autocomplete="off" >
